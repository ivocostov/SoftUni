...
class Document:
    ...
    @classmethod
    def from_instance(cls, document_id: int, category: Category, topic: Topic, file_name: str):
    ...
    def __repr__(self):
        return f"Document {self.id}: {self.file_name}; category {self.category_id}, topic {self.topic_id}, tags: {', '.join(str(t) for t in self.tags)}"
